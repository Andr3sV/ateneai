...
export default function ContactsListPage() {
  ...
  const fetchContacts = useCallback(async (page = 1, phoneFilter = '', statusFilterParam = statusFilter) => {
    ...
    try {
      ...
      if (statusFilterParam !== 'all') params.append('status', statusFilterParam)
  ...
  useEffect(() => { fetchContacts(1, searchPhone, statusFilter) }, [searchPhone, statusFilter, fetchContacts])

  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    setSearchPhone(searchValue)
  }
  const handlePageChange = (newPage: number) => { fetchContacts(newPage, searchPhone, statusFilter) }
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="name" value={newContact.name} onChange={(e) => setNewContact(prev => ({ ...prev, name: e.target.value }))} className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="phone" className="text-right">Phone</Label>
                <Input id="phone" value={newContact.phone} onChange={(e) => setNewContact(prev => ({ ...prev, phone: e.target.value }))} className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="email" className="text-right">Email</Label>
                <Input id="email" type="email" value={newContact.email} onChange={(e) => setNewContact(prev => ({ ...prev, email: e.target.value }))} className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="status" className="text-right">Status</Label>
                <Select value={newContact.status || 'Lead'} onValueChange={(value) => setNewContact(prev => ({ ...prev, status: value as Contact['status'] }))}>
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="country" value={newContact.country} onChange={(e) => setNewContact(prev => ({ ...prev, country: e.target.value }))} className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="instagram" className="text-right">Instagram</Label>
                <Input id="instagram" value={newContact.instagram_url} onChange={(e) => setNewContact(prev => ({ ...prev, instagram_url: e.target.value }))} className="col-span-3" placeholder="https://instagram.com/username" />
              </div>
            </div>
            <DialogFooter>
              <Button type="submit" onClick={async () => {
                try {
                  const data = await authenticatedFetch(getApiUrl('contacts'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newContact) })
                  if (data.success) {
                    setIsNewContactModalOpen(false)
                    setNewContact({ name: '', phone: '', email: '', status: 'Lead', country: '', instagram_url: '' })
                    fetchContacts(1, searchPhone, statusFilter)
                    fetchMetrics()
                  } else { alert('Error creating contact: ' + (data.error || 'Unknown error')) }
                } catch (error: unknown) {
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <div className="text-muted-foreground mb-4">{searchPhone || statusFilter !== 'all' ? 'Try adjusting your filters or search terms.' : 'Get started by creating your first contact.'}</div>
              {(!searchPhone && statusFilter === 'all') && (<Button onClick={() => setIsNewContactModalOpen(true)}><Plus className="h-4 w-4 mr-2" />Create First Contact</Button>)}
            </div>
          ) : (
            <>
