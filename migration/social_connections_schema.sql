...
CREATE OR REPLACE TRIGGER trigger_social_connections_updated_at
...
CREATE OR REPLACE TRIGGER trigger_webhook_configurations_updated_at
...
CREATE POLICY "Users can view their workspace social connections" ON social_connections
    FOR SELECT USING (
        workspace_id IN (
...
CREATE POLICY "Users can manage their workspace social connections" ON social_connections
    FOR ALL USING (
        workspace_id IN (
            SELECT workspace_id FROM workspace_users 
            WHERE user_id = auth.uid() 
...
CREATE POLICY "Users can view their workspace social events" ON social_events_log
    FOR SELECT USING (
        workspace_id IN (
            SELECT workspace_id FROM workspace_users 
            WHERE user_id = auth.uid()
...
CREATE POLICY "Users can view their workspace webhook configs" ON webhook_configurations
    FOR SELECT USING (
        workspace_id IN (
            SELECT workspace_id FROM workspace_users 
            WHERE user_id = auth.uid()
...
CREATE POLICY "Admins can manage workspace webhook configs" ON webhook_configurations
    FOR ALL USING (
        workspace_id IN (
            SELECT workspace_id FROM workspace_users 
            WHERE user_id = auth.uid() 
...
-- INSERT INTO webhook_configurations (workspace_id, platform, webhook_url, verify_token, app_secret, subscribed_fields) VALUES
-- (1, 'facebook', 'https://api.ateneai.com/webhooks/facebook', 'verify_token_123', 'app_secret_123', '["messages", "messaging_postbacks"]'),
-- (1, 'instagram', 'https://api.ateneai.com/webhooks/instagram', 'verify_token_456', 'app_secret_456', '["messages"]');
