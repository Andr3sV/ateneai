...
router.get('/', requireWorkspaceContext, async (req, res): Promise<void> => {
  try {
    ...
    const { from, to, status, interest, type, start_date, end_date, contact_id, assigned_user_id, unassigned, agent_id } = req.query as Record<string, string>;
...
router.put('/:id(\\d+)/status', requireWorkspaceContext, async (req, res): Promise<void> => {
  try {
    ...
    const allowed = ['mql','client','lead','agendado','no_contesta']
